                ...
                <div className="flex flex-col gap-2.5">
                  ...
                  <div className="flex items-center justify-between gap-2 bg-black/40 p-1.5 px-3 rounded-xl border border-white/10">
                    ...
                    <button
                      ...
                      className={`py-1 px-2.5 rounded-lg text-[10.5px] font-medium transition-all flex items-center justify-center gap-1 cursor-pointer active:scale-95 border flex-shrink-0 ${isCopied
                  ...
                  {trackingData.customer_name && (
                    <div className="text-[11px] sm:text-xs text-gray-400 flex items-center gap-1 px-0.5 whitespace-nowrap overflow-hidden text-ellipsis">
                      <span className="flex-shrink-0">ผู้สั่งซื้อ : </span>
                      <span className="text-white font-medium truncate">{trackingData.customer_name}</span>
                      {trackingData.customer_phone && trackingData.customer_phone !== trackingData.customer_name && (
                        <span className="text-gray-400 font-mono flex-shrink-0">({trackingData.customer_phone})</span>
                      )}
                    </div>
                  )}

                  {/* Delivery Address */}
                  {trackingData.delivery_type === 'จัดส่ง' && trackingData.address && trackingData.address !== trackingData.customer_name && (
                    <div className="text-[11px] sm:text-xs text-gray-400 flex items-start gap-1 px-0.5 pt-1 border-t border-white/5">
                      <span className="flex-shrink-0 text-gray-400">ที่อยู่จัดส่ง : </span>
                      <span className="text-gray-200 leading-snug break-words flex-1">{trackingData.address}</span>
                    </div>
                  )}
                  ...
                  <div className="space-y-1.5">
                    {steps.map((step, idx) => {
                      ...
                      return (
                        <div
                          key={step.key}
                          className={`flex gap-2.5 relative p-1.5 rounded-lg transition-all duration-200 ${state === 'active'
                        ...
                        >
                          ...
                          {!isLast && (
                            <div
                              className={`absolute left-[19px] top-[30px] bottom-[-10px] w-0.5 transition-colors duration-300 ${state === 'completed' ? 'bg-primary' : 'bg-white/10'
                                }`}
                            />
                          )}

                          {/* Stepper Circle / Number */}
                          <div className={`relative z-10 w-7 h-7 rounded-lg flex items-center justify-center text-xs transition-all duration-200 flex-shrink-0 font-mono font-bold ${state === 'completed'
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-1.5">
                              <span className={`text-[11px] sm:text-xs font-bold ${state === 'active'
                                ? 'text-primary'
                                : state === 'completed'
                                  ? 'text-white'
                                  : 'text-gray-400'
